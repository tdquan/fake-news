...
20.times do
	...
	user.email = user.first_name.downcase + "_" + user.last_name.downcase + ["email.fr", "email.com"].sample
	user.nickname = user.nickname.downcase
	user.save
end
...
COMMENTS.each do |comments|
	comments.each do |comment|
		Comment.create(content: comment, user_id: rand(1..20), post_id: Post.all.sample.id)
	end
end
